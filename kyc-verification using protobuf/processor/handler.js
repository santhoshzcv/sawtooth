...
class SmallBankHandler extends TransactionHandler {
    ...
    kyc_verification(state, investor_id,status) {
    ...
    apply(transactionProcessRequest, state) {
        ...
        console.log(payload);
        if (payload.action == protos.model.Payload.Action.UPLOAD_KYC_DATA) {
            let params = payload.upload_kyc_data;
            return this.uploadData(state, params.id, params.name, params.bankname);
        } else if (payload.action == protos.model.Payload.Action.VERIFY_KYC_DATA) {
            let params = payload.verify_kyc_data;
            console.log(params);
        
            return this.kyc_verification(state, params.id, params.status)
        }
         else {
